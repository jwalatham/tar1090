...
function webglAddLayer() {
    ...
    try {
        let glStyle = {
            symbol: {
                symbolType: 'image',
                src: 'images/sprites_1f27e6d32849ec1dc3c8429a1755c451.png',
                size: [ 'get', 'size' ],
                offset: [0, 0],
                textureCoord: [ 'array',
                    [ 'get', 'cx' ],
...
function initializeQNHSelector() {
    // Get display unit preferences from local storage
    if (!loStore.getItem('QNHUnits')) {
        loStore['QNHUnits'] = '995';
    }

    QNHUnits = loStore['QNHUnits'];

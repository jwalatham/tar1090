...
PlaneObject.prototype.getMarkerColor = function(options) {
    ...
    if ((this.dataSource == 'adsb' && this.seen_pos > 20 * 60)
        || (!globeIndex && this.dataSource != 'adsb' && this.seen_pos > 15))  {
...
PlaneObject.prototype.updateIcon = function() {
    ...
    if ( enableLabels && (!multiSelect || (multiSelect && this.selected)) &&
        ...
        let alt;
        if (labelsGeom) {
            alt = adjust_geom_alt(this.alt_baro, this.position);
        } else {
            alt = this.altitude;
        }
	let qnhnew = Math.trunc(this.nav_qnh);
	let qnhdiff = ((1013 - DisplayUnits) * 30);
	let altcalc = ((this.altitude - qnhdiff) / 100);
        let altString = (alt == null) ? unknown : format_altitude_brief (alt / 100, this.vert_rate, showLabelUnits);
        let altString2 = (alt == null) ? unknown : format_altitude_brief (altcalc, this.vert_rate);
	let altString3 = format_altitude_brief (this.alt_rounded / 100, this.vert_rate);
	let altselect = format_altitude_brief ('>' + this.nav_altitude / 100);
if (this.nav_altitude == null) {
altselect = "";
} else {
altselect = format_altitude_brief (this.nav_altitude / 100);
}
        ...
        if (uk_advisory) {
            labelText += callsign + '\n';
if (this.altitude > 6050) {
            labelText += 'FL' + altString + NBSP + this.icaoType + NBSP + '>' + altselect;
            } else {
labelText += 'A0' + altString2 + NBSP + this.icaoType + NBSP + '>' + altselect;
}
            if (this.squawk > '1729' && this.squawk < '1758' || this.squawk > '0419' && this.squawk < '0470' ) {
                    labelText += '\nLF LARS';
